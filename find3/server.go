...
func Run() (err error) {
	...
	r.POST("/battery", handlerBatteryData)
	r.POST("/asset", handlerAssetData)
...
func handlerApiV1Battery(c *gin.Context) {
	err := func(c *gin.Context) (err error) {
		...
		defer d.Close()
		voltage, err := d.GetBattery(instance)
		if err != nil {
			return
		}
...
func handlerBatteryData(c *gin.Context) {
	message, err := func(c *gin.Context) (message string, err error) {
		var b models.BatteryData
		err = c.BindJSON(&b)
		if err != nil {
			message = b.Family
			err = errors.Wrap(err, "problem binding battery data")
			return
		}

		err = b.Validate()
		if err != nil {
			err = errors.Wrap(err, "problem validating battery data")
			return
		}
		// process data
		err = api.SaveBatteryData(b)
		if err != nil {
			message = b.Family
			return
		}
		message = "inserted battery data"
		logger.Log.Debugf(" /battery %+v", b)
		return
	}(c)
	if err != nil {
		logger.Log.Debugf("[%s] problem parsing battery data: %s", message, err.Error())
		c.JSON(http.StatusOK, gin.H{"message": err.Error(), "success": false})
	} else {
...
func handlerAssetData(c *gin.Context) {
	message, err := func(c *gin.Context) (message string, err error) {
		var a models.AssetData
		err = c.BindJSON(&a)
		if err != nil {
			message = a.Family
			err = errors.Wrap(err, "problem binding asset data")
			return
		}

		err = a.Validate()
		if err != nil {
			err = errors.Wrap(err, "problem validating asset data")
			return
		}
		// process data
		err = api.SaveAssetData(a)
		if err != nil {
			message = a.Family
			return
		}
		message = "inserted asset data"
		logger.Log.Debugf(" /asset %+v", a)
		return
	}(c)
	if err != nil {
		logger.Log.Debugf("[%s] problem parsing asset data: %s", message, err.Error())
		c.JSON(http.StatusOK, gin.H{"message": err.Error(), "success": false})
	} else {
		c.JSON(http.StatusOK, gin.H{"message": message, "success": true})
	}
