...
func (d *Database) MakeTables() (err error) {
	...
	sqlStmt = `create table batteries (id INTEGER PRIMARY KEY, mac TEXT, voltage REAL);`
	_, err = d.db.Exec(sqlStmt)
	if err != nil {
		err = errors.Wrap(err, "MakeTables")
		logger.Log.Error(err)
	...
	sqlStmt = `create table assets (id INTEGER PRIMARY KEY, mac TEXT, instance TEXT);`
	_, err = d.db.Exec(sqlStmt)
	if err != nil {
		err = errors.Wrap(err, "MakeTables")
		logger.Log.Error(err)
...
// AddBattery will insert a battery data into the database
func (d *Database) AddBattery(b models.BatteryData) (err error) {
	startTime := time.Now()

	tx, err := d.db.Begin()
	if err != nil {
		return errors.Wrap(err, "AddBattery")
	}

	sqlStatement := "insert or replace into batteries (mac, voltage) values (?, ?)"
	stmt, err := tx.Prepare(sqlStatement)
	if err != nil {
		return errors.Wrap(err, "AddBattery, prepare "+sqlStatement)
	}
	defer stmt.Close()
	...
	logger.Log.Debugf("[%s] inserted battery data, %s", b.Family, time.Since(startTime))
	return
}
func (d *Database) AddAsset(b models.AssetData) (err error) {
	startTime := time.Now()

	tx, err := d.db.Begin()
	if err != nil {
		return errors.Wrap(err, "AddAsset")
	}

	sqlStatement := "insert or replace into assets (mac, instance) values (?, ?)"
	stmt, err := tx.Prepare(sqlStatement)
	if err != nil {
		return errors.Wrap(err, "AddAsset, prepare "+sqlStatement)
	}
	defer stmt.Close()
	...
	logger.Log.Debugf("[%s] inserted asset data, %s", b.Family, time.Since(startTime))
	return
}
...
func (d *Database) GetDeviceByInstance(instance string) (mac string, err error) {
	stmt, err := d.db.Prepare("SELECT mac FROM assets WHERE instance = ?")
	defer stmt.Close()
	if err != nil {
		err = errors.Wrap(err, "problem preparing SQL")
		return
	}
	err = stmt.QueryRow(instance).Scan(&mac)
	return
}
func (d *Database) GetBattery(instance string) (voltage float64, err error) {
	stmt, err := d.db.Prepare("SELECT voltage FROM batteries INNER JOIN assets ON batteries.mac=assets.mac WHERE assets.instance = ?")
	defer stmt.Close()
	if err != nil {
		err = errors.Wrap(err, "problem preparing SQL")
		return
	}
	err = stmt.QueryRow(instance).Scan(&voltage)
	return
}
