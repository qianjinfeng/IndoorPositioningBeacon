...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_CONNECTED: 
        ...
        esp_mqtt_client_publish(mqtt_client, c_presencetopic, "CONNECTED", 0, 0, 0);
        bluetooth_init();
        cJSON *root = cJSON_CreateObject();
        cJSON_AddStringToObject(root, "ip", c_ip);
        cJSON_AddStringToObject(root, "mac", c_mac);
        const char *sys_info = cJSON_Print(root);
        esp_mqtt_client_publish(mqtt_client, c_presencesubtopic, sys_info, strlen(sys_info), 0, 0);
        free((void *)sys_info);
        cJSON_Delete(root);
        }
        break;
        ...
        while (fgets(buf,sizeof(buf),fp) != NULL) {
            ...
            switch(count) {
                ...
                case 2:
                    ...
                    sprintf(c_presencetopic, "presence_nodes/%s", c_location);
                    sprintf(c_presencesubtopic, "presence_nodes/%s/tele", c_location);
                    
                    ESP_LOGI(TAG, "room topic %s", c_roomtopic);
                    ESP_LOGI(TAG, "presentce topic %s", c_presencetopic);
                    break;
                case 3:
